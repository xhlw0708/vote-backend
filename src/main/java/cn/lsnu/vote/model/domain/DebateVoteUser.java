package cn.lsnu.vote.model.domain;

import com.baomidou.mybatisplus.annotation.IdType;
import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.annotation.TableId;
import com.baomidou.mybatisplus.annotation.TableName;
import lombok.Data;

import java.io.Serializable;
import java.util.Date;

/**
 * 辩论-用户投票记录表
 * @TableName debate_vote_user
 */
@TableName(value ="debate_vote_user")
@Data
public class DebateVoteUser implements Serializable {

    /**
     * 投票状态
     */
    @TableField(exist = false)
    private Integer debateVoteStatus;

    /**
     * id
     */
    @TableId(value = "id", type = IdType.AUTO)
    private Long id;

    /**
     * 辩论投票id
     */
    @TableField(value = "debate_vote_id")
    private Long debateVoteId;

    /**
     * 用户id
     */
    @TableField(value = "user_id")
    private Long userId;

    /**
     * 用户投的小组id
     */
    @TableField(value = "group_id")
    private Long groupId;

    /**
     * 之前的投票状态 只有参与第一次投票，才能继续下一次投票 0-未参与 1-参与
     */
    @TableField(value = "pre_vote_status")
    private Integer preVoteStatus;

    /**
     * 用户投票状态 0-不允许投票 1-允许投票
     */
    @TableField(value = "vote_status")
    private Integer voteStatus;

    /**
     * 第几轮辩论赛
     */
    @TableField(value = "vote_parent_version")
    private Integer voteParentVersion;

    /**
     * 第几轮辩论赛下的每小轮
     */
    @TableField(value = "vote_children_version")
    private Integer voteChildrenVersion;

    /**
     * 创建时间
     */
    @TableField(value = "create_time")
    private Date createTime;

    /**
     * 修改时间
     */
    @TableField(value = "update_time")
    private Date updateTime;

    @TableField(exist = false)
    private static final long serialVersionUID = 1L;
}